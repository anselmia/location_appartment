...
from logement.services.email_service import send_mail_on_new_reservation, send_mail_on_refund
...
def create_stripe_checkout_session(reservation, success_url, cancel_url):
    try:
        logger.info(f"Creating Stripe checkout session for reservation {reservation.id}.")
        ...
        logger.info(f"Checkout session created successfully for reservation {reservation.id}. Session ID: {session.id}")
        return session
...
def user_has_saved_card(stripe_customer_id):
    try:
        logger.info(f"Checking for saved cards for Stripe customer {stripe_customer_id}.")
...
def create_stripe_checkout_session_with_deposit(reservation, success_url, cancel_url):
    try:
        logger.info(f"Creating Stripe checkout session with deposit for reservation {reservation.id}.")
        ...
        if customer_id:
            ...
            logger.info(f"Checkout session with deposit created successfully for reservation {reservation.id}.")
            return {
                "checkout_session_url": checkout_session.url,
                "session_id": checkout_session.id,
            }
        else:
            logger.warning(f"No customer ID returned from Stripe for user {reservation.user.id}.")
...
def create_stripe_customer_if_not_exists(user):
    if not user.stripe_customer_id:
        try:
            ...
            logger.exception(f"Failed to create Stripe customer for user {user.id}: {e}")
            raise
...
def refund_payment(reservation, amount_cents=None):
    try:
        ...
        logger.info(f"Refund successfully processed for reservation {reservation.id}. Refund ID: {refund.id}")
        return refund
...
def handle_charge_refunded(data):
    ...
    try:
        ...
        if refunds:
            ...
            logger.info(f"💶 Refund ID: {refund_id}, Amount: {refunded_amount:.2f} {currency}")
...
def handle_payment_intent_succeeded(data):
    ...
    if payment_type == "deposit":
        ...
        logger.info(f"✅ Dépôt reçu: {amount:.2f} € via PaymentIntent {payment_intent_id}")
...
def handle_checkout_session_completed(data):
    ...
    logger.info(f"🔔 Handling checkout.session.completed for reservation {reservation_id}")
    ...
    try:
        ...
        if reservation.statut != "confirmee":
            try:
                ...
                if not payment_method:
                    raise ValueError(f"No payment method found on PaymentIntent {payment_intent_id}")

                if not reservation.stripe_saved_payment_method_id:
                    reservation.stripe_saved_payment_method_id = payment_method.id
                    logger.info(f"💾 Saved payment method {payment_method.id} for reservation {reservation.id}")

            except Exception as e:
                logger.exception(f"❌ Failed to retrieve or validate payment method: {e}")
                raise
        ...
        logger.exception(f"❌ Unexpected error handling checkout.session.completed: {e}")
        raise
...
def charge_payment(saved_payment_method_id, amount_cents, customer_id, reservation, currency="eur"):
    try:
        ...
        logger.info(f"Charge payment successful for reservation {reservation.id}.")
        return intent
